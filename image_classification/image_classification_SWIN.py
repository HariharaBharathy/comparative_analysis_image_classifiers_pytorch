...
class ClassifierNetwork(nn.Module):
    """
            Backbone of classifier is SWIN - Transformer for vision problems
    """

    def __init__(self):
        super(ClassifierNetwork, self).__init__()

        self.swin_base = torchvision.models.swin_v2_t(weights='Swin_V2_T_Weights.IMAGENET1K_V1')
        self.swin_base.head = nn.Linear(in_features=768, out_features=196, bias=True)
        self.swin_base.num_classes = 196

    def forward(self, input):
        # get two images' features
        output = self.swin_base(input)
        return output
...
def main():
    ...
    parser.add_argument('--experiment-name', type=str, default="Classify_stanford_with_SWIN_pretrained", metavar='N',
